...
export function DoDwhSpec(battleLog: BattleLog, attacker:PlayerBonuses, target:Monster, tick:number):void {
    let specRoll:number = Math.random()*adUtil.CalculatePlayerAttackRoll(attacker.effectiveAttackLevel, attacker.totalCrushBonus);
    let defRoll:number = Math.random()*adUtil.CalculateNpcDefRoll(target.defenceLevel, target.crushBonus);
...
export function DoBgsSpec(battleLog: BattleLog, attacker:PlayerBonuses, target:Monster, tick:number):void {
    let specRoll:number = Math.random()*adUtil.CalculatePlayerAttackRoll(attacker.effectiveAttackLevel, attacker.totalSlashBonus)*2;
    let defRoll:number = Math.random()*adUtil.CalculateNpcDefRoll(target.defenceLevel, target.slashBonus);
    let damageRoll:number = rolls.rollForDamage(
